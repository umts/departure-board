...
function alertIsActive(alert) {
  return (
    ...
    alert.activePeriod.some((timePeriod) => {
      return (
        (timePeriod.start === 0 || isPast(fromUnixTime(timePeriod.start))) &&
...
function alertIsRelevant(alert, stopIds, routeIds) {
  return alert.informedEntity.some(
    (informedEntity) =>
      informedEntity.agencyId ||
      stopIds.includes(informedEntity.stopId) ||
      routeIds.includes(informedEntity.routeId),
  );
}
  ...
  return [
    ...new Set(
      alert.informedEntity
        .filter((informedEntity) => !!informedEntity.routeId)
        .map((informedEntity) => routesById[informedEntity.routeId]),
    ),
  ].filter((route) => !!route);
}
